...
If you prefer `uv` (same workflow as the tau-bench sample), you can recreate the pinned `.venv` via:
...
1) **Prereqs**
   ...
   - Env vars:  
     - Either set `PUBLIC_URL` directly or provide `CLOUDRUN_HOST` (+ optional `CLOUDRUN_PATH`) and `HTTPS_ENABLED=true` to build it automatically.  
...
2) **Run controllers (green/white)**
   - Green:
     ```bash
     cd agentbeats_v2
     HTTPS_ENABLED=true CLOUDRUN_HOST=yourdomain.com CLOUDRUN_PATH=/green \
     AGENT_HOST=0.0.0.0 AGENT_PORT=8010 \
     python main.py green   # or set PUBLIC_URL=https://yourdomain/green instead
     ```
   - White (optional echo agent):
     ```bash
     HTTPS_ENABLED=true CLOUDRUN_HOST=yourdomain.com CLOUDRUN_PATH=/white \
     AGENT_HOST=0.0.0.0 AGENT_PORT=8011 \
     python main.py white   # or set PUBLIC_URL=https://yourdomain/white instead
     ```
...
- If `CLOUDRUN_HOST` is set, the code defaults `CLOUDRUN_PATH` to `/green` or `/white` so you do not have to repeat it unless your ingress expects a different path.

## Controller mode (`agentbeats run_ctrl`)

For a Cloud Run–style controller (mirrors the sample tau-bench flow):
```bash
cd agentbeats_v2
HTTPS_ENABLED=true CLOUDRUN_HOST=your.domain ROLE=green agentbeats run_ctrl
```
This uses the local `run.sh` to start the green agent; the controller proxies it via `/to_agent/<id>/...` and exposes `/status`/`/agents`. Set `HOST`/`PORT` if you need non-default bind addresses.
